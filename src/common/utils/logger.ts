...
const createBaseLogger = (moduleLabel?: string): Logger => {
    const logger = createLogger({
        ...
        format: fileFormat,
        defaultMeta: { service: 'AST-Server' },
        transports: [
                new transports.File({ filename: path.join(process.cwd(), 'logs', 'app.log'), level: 'error' }),
                new transports.File({ filename: path.join(process.cwd(), 'logs', 'app.log'), level: 'combine.log' })
        ]
    })
...
export const getModuleLogger = (moduleName: string): Logger => {
    // potentially look up dynamic log levels here if needed
